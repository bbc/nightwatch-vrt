...
function defaultScreenshotFilePath(nightwatchClient, basePath, fileName) {
    const moduleName = nightwatchClient.currentTest.module,
        ...
        browserName = nightwatchClient.capabilities.browserName + ' ' + nightwatchClient.capabilities.platform
    var screenShotName = appendExtensionToFilename(fileName || kebabName)

        // If we are using a section/element we want to ensure we pick the right part 
    if (screenShotName.indexOf('@') > -1) {
        screenShotName = appendExtensionToFilename(
                screenShotName.substring(screenShotName.lastIndexOf('@'), 
                screenShotName.lastIndexOf(']'))
            );
    }

    return path.join(process.cwd(), basePath, browserName, moduleName, testName + ' ' + screenShotName)
}
